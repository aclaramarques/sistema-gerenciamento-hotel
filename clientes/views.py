...
class HospedeListCreateView(APIView):
    ...
    def post(self, request):
        ...
        # Tenta buscar um hóspede existente pelo CPF
        try:
            hospede = Hospede.objects.get(cpf=cpf)
            # Se encontrou, ATUALIZA os dados (ex: usuário mudou telefone)
            serializer = HospedeCreateUpdateSerializer(hospede, data=data, partial=True)
            if serializer.is_valid():
                serializer.save()
                # Retorna 200 OK com o ID (CPF) para o Flutter prosseguir
                return Response({'id': hospede.cpf}, status=status.HTTP_200_OK)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

        except Hospede.DoesNotExist:
            # Se não existe, CRIA um novo
            serializer = HospedeCreateUpdateSerializer(data=data)
            if serializer.is_valid():
                obj = serializer.save()
                return Response({'id': obj.cpf}, status=status.HTTP_201_CREATED)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)